...
def record_audio(filename):
    ...
    fs = 44100
    seconds = 10
    RecordingData = sd.rec(int(seconds * fs), samplerate=fs, channels=2)
    global recording
    recording = True
    sd.wait()
    save_audio(RecordingData, fs, filename=filename)
...
def create_gui():
    ...
    global RecordingData
    Save_Button = tk.Button(root, text="Save", command=lambda: save_audio(RecordingData, 44100, "./audio.wav"))
